...
public class SDK_WebApp extends Activity {
 ...
 class WebappMode implements ICoreStatusListener,IOnCreateSplashView {
	...
	public void onCoreInitEnd(ICore coreHandler) {
		...
		//创建默认webapp，赋值appid
		String appBasePath = "/apps/After";//必须和assets\apps的项目名一致
		...
		app = SDK.startWebApp(activity,appBasePath, args, new IWebviewStateListener() {
			...
			public Object onCallBack(int pType, Object pArgs) {
				...
				case IWebviewStateListener.ON_WEBVIEW_READY:
					WebView webView = ((IWebview)pArgs).obtainWebview();
					webView.getSettings().setDomStorageEnabled(true);
					webView.getSettings().setJavaScriptEnabled(true);
					webView.addJavascriptInterface(new ClockServerImpl(activity), "android");
	...
	public void onCloseSplash() {
		...
		AlarmUtil alarmUtil = new AlarmUtil(activity);
		alarmUtil.joinWhiteList();
	}
}
